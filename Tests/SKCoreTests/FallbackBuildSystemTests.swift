...
final class FallbackBuildSystemTests: XCTestCase {
  ...
  func testSwift() throws {
    ...
    let settings = bs.settings(for: source.asURI, language: .swift)!
    ...
    XCTAssertEqual(bs.settings(for: source.asURI, language: .swift)?.compilerArguments, [
      source.pathString,
    ])
  }
  ...
  func testSwiftWithCustomFlags() throws {
    ...
    let args = bs.settings(for: source.asURI, language: .swift)?.compilerArguments
    XCTAssertEqual(args, [
      "-Xfrontend",
      "-debug-constraints",
      "-sdk",
  ...
  func testCXX() throws {
    ...
    let settings = bs.settings(for: source.asURI, language: .cpp)!
    XCTAssertNil(settings.workingDirectory)
    ...
    XCTAssertEqual(bs.settings(for: source.asURI, language: .cpp)?.compilerArguments, [
      source.pathString,
    ])
  }
  ...
  func testC() throws {
    ...
    let bs = FallbackBuildSystem(buildSetup: .default)
    bs.sdkpath = nil
    XCTAssertEqual(bs.settings(for: source.asURI, language: .c)?.compilerArguments, [
      source.pathString,
    ])
  }
  ...
  func testObjC() throws {
    ...
    let bs = FallbackBuildSystem(buildSetup: .default)
    bs.sdkpath = nil
    XCTAssertEqual(bs.settings(for: source.asURI, language: .objective_c)?.compilerArguments, [
      source.pathString,
    ])
  }
  ...
  func testObjCXX() throws {
    ...
    let bs = FallbackBuildSystem(buildSetup: .default)
    bs.sdkpath = nil
    XCTAssertEqual(bs.settings(for: source.asURI, language: .objective_cpp)?.compilerArguments, [
      source.pathString,
    ])
  }
  ...
  func testUnknown() throws {
    let source = try AbsolutePath(validating: "/my/source.mm")
    let bs = FallbackBuildSystem(buildSetup: .default)
    XCTAssertNil(bs.settings(for: source.asURI, language: Language(rawValue: "unknown")))
  }
}
