...
final class BuildSystemManagerTests: XCTestCase {
  ...
  func testSettingsMainFile() async throws {
    ...
    assertEqual(await bsm.buildSettings(ofMainFileFor: a, language: .swift)?.buildSettings, bs.map[a]!)
  ...
  func testSettingsMainFileInitialNil() async throws {
    ...
    let del = await BSMDelegate(bsm)
    await bsm.registerForChangeNotifications(for: a, language: .swift)
    assertNil(await bsm.buildSettings(ofMainFileFor: a, language: .swift)?.buildSettings)
  ...
  func testSettingsMainFileWithFallback() async throws {
    ...
    await bsm.registerForChangeNotifications(for: a, language: .swift)
    assertEqual(await bsm.buildSettings(ofMainFileFor: a, language: .swift)?.buildSettings, fallbackSettings)
  ...
  func testSettingsMainFileInitialIntersect() async throws {
    ...
    assertEqual(await bsm.buildSettings(ofMainFileFor: b, language: .swift)?.buildSettings, bs.map[b]!)
  ...
  func testSettingsHeaderChangeMainFile() async throws {
    ...
    assertEqual(await bsm.buildSettings(ofMainFileFor: h, language: .c)?.buildSettings, bs.map[cpp1]!)
  ...
  func testSettingsOneMainTwoHeader() async throws {
    ...
    assertEqual(await bsm.buildSettings(ofMainFileFor: h1, language: .c)?.buildSettings, expectedArgsH1)
    assertEqual(await bsm.buildSettings(ofMainFileFor: h2, language: .c)?.buildSettings, expectedArgsH2)
  ...
  func testSettingsChangedAfterUnregister() async throws {
    ...
    assertEqual(await bsm.buildSettings(ofMainFileFor: a, language: .swift)?.buildSettings, bs.map[a]!)
    assertEqual(await bsm.buildSettings(ofMainFileFor: b, language: .swift)?.buildSettings, bs.map[b]!)
    assertEqual(await bsm.buildSettings(ofMainFileFor: c, language: .swift)?.buildSettings, bs.map[c]!)
