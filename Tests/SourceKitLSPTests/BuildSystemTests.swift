...
final class TestBuildSystem: BuildSystem {
  ...
  func settings(for document: DocumentURI, language: Language) async throws -> FileBuildSettings? {
...
final class BuildSystemTests: XCTestCase {
  ...
  func testSwiftDocumentUpdatedBuildSettings() async throws {
    ...
    let doc = DocumentURI(url)
    let args = FallbackBuildSystem(buildSetup: .default).settings(for: doc, language: .swift)!.compilerArguments
  ...
  func testSwiftDocumentFallbackWithholdsSemanticDiagnostics() async throws {
    ...
    var primarySettings = FallbackBuildSystem(buildSetup: .default).settings(for: doc, language: .swift)!
