...
// Copy pasted from PackageLoading/Platform.swift to make it concurrency-safe

extension Platform {
  // This is not just a computed property because the ToolchainRegistryTests
  // change the value.
  public nonisolated(unsafe) static let currentConcurrencySafe: Platform? = {
    #if os(Windows)
    return .windows
    #else
  ...
  private static func findCurrentPlatformLinux(_ fileSystem: FileSystem) -> Platform? {
    do {
      ...
      if try fileSystem.isFile(AbsolutePath(validating: "/etc/redhat-release"))
        ...
        || fileSystem.isFile(AbsolutePath(validating: "/etc/fedora-release")) || Platform.isAmazonLinux2(fileSystem)
      {
        return .linux(.fedora)
      }
    } catch {}
