...
public actor SwiftLanguageServer: ToolchainLanguageServer {
  ...
  public let queue2: DispatchQueue = DispatchQueue(label: "swift-language-server-queue", qos: .userInitiated)
  ...
  func buildSettings(for document: DocumentURI) async -> SwiftCompileCommand? {
    ...
    let buildSettingsChange = await workspace.buildSystemManager.settings(for: document, language: .swift)
    return SwiftCompileCommand(change: buildSettingsChange)
  }
  ...
  private func requestTokensRefresh() {
    if capabilityRegistry.clientHasSemanticTokenRefreshSupport {
      _ = client.send(WorkspaceSemanticTokensRefreshRequest(), queue: queue2) { result in
  ...
  func publishDiagnostics(
    ...
    let diagnostics = registerDiagnostics(
      ...
      snapshot: snapshot,
      stage: stage,
      isFallback: compileCommand?.isFallback ?? true
    )
...
extension SwiftLanguageServer {
  ...
  public func _documentDiagnostic(
    ...
    let handle = self.sourcekitd.send(skreq, self.queue2) { response in
  ...
  public func documentDiagnostic(
    ...
    await self._documentDiagnostic(uri, completion)
  }
  ...
  func applyEdit(label: String, edit: WorkspaceEdit, completion: @escaping (LSPResult<ApplyEditResponse>) -> Void) {
    ...
    let handle = client.send(req, queue: queue2) { reply in
      switch reply {
