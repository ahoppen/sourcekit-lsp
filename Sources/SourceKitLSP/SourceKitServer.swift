...
extension SourceKitServer: MessageHandler {
  public nonisolated func handle(_ params: some NotificationType, from clientID: ObjectIdentifier) {
    // All of the notifications sourcekit-lsp currently handles can modify the
    // might modify the global state (eg. whether a document is open or its
    // contents in a way that) changes the results of requsts before and after.
