...
public actor SourceKitServer {
  ...
  func languageService(for uri: DocumentURI, _ language: Language, in workspace: Workspace) async -> ToolchainLanguageServer? {
    ...
    logger.info("Using toolchain \(toolchain.displayName) (\(toolchain.identifier)) for \(uri.loggable)")
...
extension SourceKitServer: MessageHandler {
  ...
  private nonisolated func _logRequest<R>(_ request: Request<R>) {
    logger.log("Received request: \(request.loggable)")
  }

  private func _logNotification<N>(_ notification: Notification<N>) {
    logger.log("Received notification: \(notification.loggable)")
  }
  ...
  public func filesDependenciesUpdated(_ changedFiles: Set<DocumentURI>) async {
    ...
    for workspace in self.workspaces {
      ...
      for uri in self.affectedOpenDocumentsForChangeSet(changedFilesForWorkspace, self.documentManager) {
        logger.log("Dependencies updated for opened file \(uri.loggable)")
        if let service = workspace.documentService[uri] {
...
extension SourceKitServer {
    ...
    if case .dictionary(let options) = req.initializationOptions {
      ...
      if case .dictionary(let completionOptions) = options["completion"] {
        ...
        case .some(let invalid):
          logger.warning("expected null or int for 'maxResults'; got \(String(reflecting: invalid))")
        }
      }
    }
  ...
  private func dynamicallyRegisterCapability(
    ...
    let _ = client.send(req) { result in
      if let error = result.failure {
        logger.error("Failed to dynamically register for \(registration.method): \(error.loggable)")
  ...
  nonisolated func cancelRequest(_ notification: CancelRequestNotification) {
    ...
    cancellationMessageHandlingQueue.async(priority: .high) {
      guard let task = await self.inProgressRequests[notification.id] else {
        logger.fault("Cannot cancel request \(notification.id) because it hasn't been scheduled for execution yet")
        return
      }
      task.cancel()
    }
  ...
  func openDocument(_ notification: DidOpenTextDocumentNotification) async {
    ...
    guard let workspace = await workspaceForDocument(uri: uri) else {
      logger.error("received open notification for file '\(uri.loggable)' without a corresponding workspace, ignoring...")
      return
    }
  ...
  func closeDocument(_ notification: DidCloseTextDocumentNotification) async {
    let uri = notification.textDocument.uri
    guard let workspace = await workspaceForDocument(uri: uri) else {
      logger.error("received close notification for file '\(uri.loggable)' without a corresponding workspace, ignoring...")
      return
    }
  ...
  func changeDocument(_ notification: DidChangeTextDocumentNotification) async {
    ...
    guard let workspace = await workspaceForDocument(uri: uri) else {
      logger.error("received change notification for file '\(uri.loggable)' without a corresponding workspace, ignoring...")
      return
    }
