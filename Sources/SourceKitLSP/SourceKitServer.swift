...
public actor SourceKitServer {
  // FIXME: (async) We shouldn't need this at all if client.send doesn't take a queue anymore.
  ...
  func languageService(
    ...
    for languageService in languageServices[serverType, default: []] {
      if await languageService.canHandle(workspace: workspace) {
...
extension SourceKitServer {
  ...
  private func workspace(uri: DocumentURI) async -> Workspace? {
  ...
  func initialize(_ req: Request<InitializeRequest>) async {
    ...
    if let workspaceFolders = req.params.workspaceFolders {
      self.workspaces.append(contentsOf: await workspaceFolders.asyncCompactMap { await self.workspace(uri: $0.uri) })
    } else if let uri = req.params.rootURI {
      if let workspace = await self.workspace(uri: uri) {
        self.workspaces.append(workspace)
      }
    } else if let path = req.params.rootPath {
      if let workspace = await self.workspace(uri: DocumentURI(URL(fileURLWithPath: path))) {
        self.workspaces.append(workspace)
      }
    }
  ...
  func didChangeWorkspaceFolders(_ note: Notification<DidChangeWorkspaceFoldersNotification>) async {
    ...
    if let added = note.params.event.added {
      let newWorkspaces = await added.asyncCompactMap { await self.workspace(uri: $0.uri) }
